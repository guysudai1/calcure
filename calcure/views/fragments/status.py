...
class TaskStatusView(View):
    ...
    def render(self):
        ...
        self.display_line(self.y - 1, self.x, color=Color.TITLE, bold=True, text=title_message)

        not_started_status = f"Not started yet" 
        not_started_indent = self.x + len(title_message) + self.SEPERATOR_SIZE
        self.display_line(self.y - 1,not_started_indent, color=Color.NOT_STARTED, bold=True, text=not_started_status)

        wip_status = f"Work in progress" 
        wip_indent = not_started_indent + len(not_started_status) + self.SEPERATOR_SIZE
        self.display_line(self.y - 1,wip_indent, color=Color.WIP, bold=True, text=wip_status)

        current_mission_status = f"Current mission" 
        current_mission_indent = wip_indent + len(wip_status) + self.SEPERATOR_SIZE
        self.display_line(self.y - 1,current_mission_indent, color=Color.CURRENT_MISSION, bold=True, text=current_mission_status)

        waiting_status = f"Waiting" 
        waiting_indent = current_mission_indent + len(current_mission_status) + self.SEPERATOR_SIZE
        self.display_line(self.y - 1,waiting_indent, color=Color.WAITING, bold=True, text=waiting_status)
